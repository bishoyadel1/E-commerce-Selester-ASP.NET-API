    ...
    public class OrderRepository : IOrderRepository
    {
        ...
        public async Task<bool> AddNewOrder(int userId, int addressId,string payMethod, string phoneNumebr)
        {
            ...
            foreach(var product in cartProducts)
            {
               ...
               var sellerId =  context.Products.First(i => i.Id == product.ProductId).UserId;

                var sellerProfit = context.SellerProfits.FirstOrDefault(o => o.UserId == sellerId);
                if (sellerProfit != null && sellerProfit.Date.Date.Month == DateTime.Today.Month)
                {
                    sellerProfit.ProductCount += product.ProductQuantity;
                    sellerProfit.ProfitCount += product.TotalPrice;
                   context.SellerProfits.Update(sellerProfit);
                }
                else
                {
                    context.SellerProfits.Add(new SellerProfit
                    {
                        UserId= sellerId,
                        ProductCount = product.ProductQuantity,
                        Date = DateTime.Now,
                        ProfitCount = product.TotalPrice
                    });
