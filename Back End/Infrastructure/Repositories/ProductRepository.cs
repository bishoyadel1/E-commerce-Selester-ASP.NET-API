...
namespace Infrastructure.Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        public ICollection<Product> GetAllWithSpec(QueryParametars parametars)
        {
            ...
            {
                //var BALL = context.Categories.Where(i => i.ID == parametars.categoryid).FirstOrDefault().ToString().ToLower(); 
                if (parametars.categoryid != 1002) { products = products.Where(p => p.CategoryID == parametars.categoryid).ToList(); }
            ...
            {
                //if(context.Brands.Where(i => i.ID == parametars.brandId).FirstOrDefault().Name !=  "all") { products = products.Where(p => p.BrandID == parametars.brandId).ToList(); }

                if(parametars.brandId!= 2002) { products = products.Where(p => p.BrandID == parametars.brandId).ToList(); }
               

            }
            if (parametars.UserId > 0)
            {
                products = products.Where(p => p.UserId == parametars.UserId).ToList();
            }
            ...
            {
                var searchTerm = parametars.Search.ToLower(); 
