            ...
            if (ModelState.IsValid)
            {
                try
                {
                   var userIdFromToken = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier)?.Value;
                    ...
                    CommentClassificationModelMl.ModelInput CommentClassificationData = new CommentClassificationModelMl.ModelInput()
                    {
                        Col0 = reviewDto.Text,
                    };
                    var sortedScoresWithLabel = CommentClassificationModelMl.PredictAllLabels(CommentClassificationData);
                    foreach (var score in sortedScoresWithLabel)
                    {


                       var sc = score.Value;
                        if (score.Key == "0" && score.Value> 0.50)
                            isToxic = true;
                    }
